...
namespace FSF.Thullo.Core.Interfaces.DataAccess
{
  ...
  {
    // Boards
    Board CreateBoard(IDbConnection db, Guid userId, Board board);
    ...
    // Lists
    List CreateList(IDbConnection db, Guid userId, int boardId, List list);
    ...
    // Cards
    Card CreateCard(IDbConnection db, Guid userId, int boardId, int listId, Card card);
